...
const MYFundingPage = () => {
  ...
  //       setStoryList(
  //         StoryList.concat(
  //           d.findUsersCheeredPerformances.map((fcp: any) => fcp.story),
  //         ),
  //       );
  //     });
  //   },
  ...
  const { data } = useSWR(getQueries.getUserCheeredPF(userData.id), fetcher);
    ...
    <>
      {data ? (
        <StoryListView
          ...
          storyData={data.findUsersCheeredPerformances.map((d: any) => d.story)}
          // pageIndex={pageIndex}
